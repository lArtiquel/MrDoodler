	...
	while (app.isOpen())                         //пока окно открыто-играем
	{
		...
		if (y < h)                                                        //если персонаж ускакал вверх, платформу переставляем на новое место вверху
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (plat[i].y > 533)
				{
					...
					if (platHasAttachedEnemy == 0) { a = 0; plat[i].x = rand() % 370; }
					else
					{
					...
					if (score >= 100 && score < 150) { plat[i].typePlatform = rand() % (2); }
					else  //рандомим тип платформы 0-обычная,1-ломающаяся, 2-горизонтальный лифт, 3-вертикальный лифт
						if (score >= 150 && score < 200) { plat[i].typePlatform = rand() % (3); }
						else
					...
					case 0: forRandomNumber = rand() % (100) + 1;
						if (forRandomNumber > 43) { plat[i].attribyte_type = 0; }
						else
							if (forRandomNumber <= 15) { plat[i].attribyte_type = 1; plat[i].selectPosition_y = plat[i].x + rand() % (43) + 3; plat[i].bufferPosition_y = plat[i].y - 12; }
							else
								if (forRandomNumber > 15 && forRandomNumber <= 25) { plat[i].attribyte_type = 2; plat[i].selectPosition_y = plat[i].x + 12; plat[i].bufferPosition_y = plat[i].y - 14; }
								else
									if (forRandomNumber > 25 && forRandomNumber <= 35) { plat[i].attribyte_type = 3; plat[i].selectPosition_y = plat[i].x + 10; plat[i].bufferPosition_y = plat[i].y - 15; plat[i].drawObject = 1; }
									else
										if (forRandomNumber > 35 && forRandomNumber <= 40) { plat[i].attribyte_type = 4; plat[i].selectPosition_y = plat[i].x + 12; plat[i].bufferPosition_y = plat[i].y - 46; plat[i].drawObject = 1; }
										else
		...
		if (q == 1)                                                         //взаимодействие с платформой и вражинами ПРАВОГО ДУДЛЕРА
		{
			...
			for (int i = 0; i<3; i++)                                           //проверка на столкневение с вражинами
			{
				...
				case 1: if (x + 40>enemy[i].x && x - 52<enemy[i].x && y + 55>enemy[i].y && y - 40<enemy[i].y
					&& drawDoodlerWithObject == 0 && dy >= -5) {
					doGameOver(app, score, close, music); return;
				} break;
				case 2: if (x + 50>enemy[i].x && x - 55<enemy[i].x && y + 50 >= enemy[i].y && y + 38 <= enemy[i].y && dy>0) { dy = -18; enemy[i].isEnemyExists = 0; }
						else   //можно подпрыгнуть на крышке нло
		...
		else
		{
			...
			for (int i = 0; i<3; i++)                                 //проверка на столкновение с вражиной левого дудла
			{
				...
				case 2: if (x + 58>enemy[i].x && x - 48<enemy[i].x && y + 50 >= enemy[i].y && y + 38 <= enemy[i].y && dy>0) { dy = -18; enemy[i].isEnemyExists = 0; }
						else   //можно подпрыгнуть на крышке нло
		...
		{
		case 0: break;
		case 1: if (q == 1) { sHat.setPosition(x + 11, y + 2); app.draw(sHat); }
				else { sHat.setPosition(x + 23, y + 2); app.draw(sHat); }
				if (dy >= -5)
				{
					drawDoodlerWithObject = 0; renderingUsedObject = 1;
					if (q == 1) { buffer_y = y + 2; buffer_x = x + 10; }
					else { buffer_y = y + 1; buffer_x = x + 23; }
				} break;
		case 2: if (q == 1) { sFlyingRocket.setPosition(x, y + 14); app.draw(sFlyingRocket); }
				else { sFlyingRocket.setPosition(x + 69, y + 14); app.draw(sFlyingRocket); }
		...
		for (int i = 0; i < 10; i++)                                         //рисуем и двигаем платформу в зависимости от ее типа
			...
			case 0:                                                          //обычная зеленая 
				if (plat[i].attribyte_type == 1) { sSpring.setPosition(plat[i].selectPosition_y, plat[i].bufferPosition_y); app.draw(sSpring); }
				else
					if (plat[i].attribyte_type == 2) { sTramp.setPosition(plat[i].selectPosition_y, plat[i].bufferPosition_y); app.draw(sTramp); }
					else
						if (plat[i].attribyte_type == 3) { if (plat[i].drawObject == 1) { sHat.setPosition(plat[i].selectPosition_y, plat[i].bufferPosition_y); app.draw(sHat); } }
						else
							if (plat[i].attribyte_type == 4) { if (plat[i].drawObject == 1) { sRocket.setPosition(plat[i].selectPosition_y, plat[i].bufferPosition_y); app.draw(sRocket); } }
							else
